...
- `--mem=131072` (128MB in KB) and a process-count cap replace the current `--memory 128m --cpus 0.5` Docker flags; no `--share-net` flag passed (isolate default = no network), matching current `--network none` (spec: Design).
