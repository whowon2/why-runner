...
import { useDeleteLessonConstraint } from "@/hooks/use-delete-lesson-constraint";
...
// Solution constraints are a lesson-only feature: they attach to one lesson
// entry (one problem inside one track), never to the problem itself, and
// never apply to contests or standalone practice submissions.
export function LessonConstraintsPanel({ lessonId }: { lessonId: string }) {
  const t = useTranslations("RoadmapPage.Constraints");
  const { data: constraints, isPending } = useLessonConstraints(lessonId);
...
function StructuralConstraintRow({
  ...
  function handleSave() {
    ...
    update(
      { id: constraint.id, lessonId, ruleType, ruleParams },
