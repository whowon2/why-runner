...
The system SHALL display the problems list as a table with columns for problem name, number of users who have solved the problem, and whether the current user has solved it.

#### Scenario: Table columns render
- **WHEN** an authenticated user opens the problems list page
- **THEN** the system SHALL render a table with one row per problem, showing the problem's name, its solved-by count, and a solved/unsolved indicator for the current user in that row
...
- **THEN** the table SHALL update to reflect the filtered/paginated set exactly as the previous list view did
