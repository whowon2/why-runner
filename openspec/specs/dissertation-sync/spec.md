...
Keeps the thesis documents (`overleaf/main.tex` and `overleaf/apps-edu.tex`) synchronized with what WhyRunner has actually shipped: the Classes/Lessons/Exercises authoring model, AI-assisted authoring capabilities, pre-publish validation and solution-constraint enforcement, per-exercise grading and notifications, and an honest scoping of what the classroom evaluation trial did and did not validate — so the dissertation neither undersells shipped features nor overclaims what was classroom-tested, and the Portuguese white paper stays in parity with the thesis.
