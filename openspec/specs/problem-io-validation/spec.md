...
Defines validation of a draft problem's reference solution against its declared input/output pairs before publishing, run through the same sandboxed judge execution used for student submissions, isolated from student-facing data.

## Requirements

### Requirement: Reference solution input
The problem edit workspace SHALL let the creator of a draft problem enter a reference solution: source code and a language selection from the same set of languages submissions support.

#### Scenario: Reference solution persisted with the draft
- **WHEN** the creator enters reference solution code and selects a language, then saves the draft
- **THEN** the reference solution code and language are persisted alongside the draft problem

### Requirement: Triggering a validation run
The creator SHALL be able to trigger a validation run for their draft problem, which executes the current reference solution against the problem's current declared input/output pairs through the same sandboxed judge execution used for student submissions.
...
### Requirement: Validation results are isolated from student-facing data
Validation runs SHALL be stored separately from student submissions and SHALL NOT appear in leaderboards, submission history, contest management views, or contest exports.

#### Scenario: Validation run does not appear as a submission
- **WHEN** a validation run is created, executed, and completed for a problem
- **THEN** no row is added to the student-facing submission data used by leaderboards, per-user submission history, or contest exports

### Requirement: Per-test validation results
Once a validation run completes, the system SHALL display, per declared input/output pair, whether the reference solution's output matched the declared output, and for any mismatch, the declared (expected) output alongside the reference solution's actual output.
...
A passing validation run SHALL be considered valid only for the exact input/output content it ran against. If the problem's input/output pairs are changed after a passing run, the system SHALL treat that run as stale and no longer count it as a passing validation for this problem.
