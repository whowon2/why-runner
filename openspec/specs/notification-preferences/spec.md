...
Defines how users control which notification types they receive: an enabled-by-default model requiring no per-user backfill, a settings UI grouping every type by module, and enforcement of the recipient's preference at notification-creation time across all trigger paths, including ones outside the web application process.
