...
**Followers/following list pages reuse the follow graph's cursor-pagination shape.** `getFollowList({ username, tab: "followers" | "following", cursor, limit, query })` queries `userFollow` joined to `user`, ordered by `createdAt desc, followerId/followingId` tiebreaker — same pattern as `getFeed`, mirroring davar's `/api/users/[username]/followers` + `FollowList` component (list rows: avatar, name, username, per-row `FollowButton`). Implemented as a server action, not a route, per WhyRunner's server-action-only convention. Profile follower/following counts link to `/user/[username]/followers` and `/user/[username]/following`.

**Search bar on the followers/following list pages filters server-side, not client-side.** Since the list is already cursor-paginated (not fully loaded into the DOM), filtering client-side would only search the currently-loaded page. `getFollowList` takes an optional `query` param and does an `ilike` match against `name`/`username` in the same joined query, keeping one code path instead of a separate search endpoint. The list component debounces input (davar has no equivalent search on `FollowList`; this is new relative to the reference) and resets pagination on query change via the React Query key.
